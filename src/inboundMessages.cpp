  ...
  switch(messageType) {
    case SF_STAGE_MESSAGE: //Current stage solid fuel
      ...
      if (msgSize == sizeof(resourceMessage)){
        ...
        if (mySStageFuel.available == 0){
          led.show_in_bar(Bar_Device, 3, 0);
        }
        else {
          led.show_in_bar(Bar_Device, 3, map(mySStageFuel.available, 0, mySStageFuel.total, 1, 10));
        }      
      }
        break;
    case LF_STAGE_MESSAGE: //Current stage liquid fuel
      if (msgSize == sizeof(resourceMessage)){
        ...
        if (myLStageFuel.available == 0){
          led.show_in_bar(Bar_Device, 2, 0);
        }
        else {
          led.show_in_bar(Bar_Device, 2, map(myLStageFuel.available, 0, myLStageFuel.total, 1, 10));
        }
      }
      break;
    case MONO_MESSAGE: //Vessel total Monopropellant fuel
      if (msgSize == sizeof(resourceMessage)){
        ...
        if (myMonoFuel.available == 0){
          led.show_in_bar(Bar_Device, 1, 0);
        }
        else {
          led.show_in_bar(Bar_Device, 1, map(myMonoFuel.available, 0, myMonoFuel.total, 1, 10));
        }
      }
      break;
    case ELECTRIC_MESSAGE: //Vessel total Battery charge
      if (msgSize == sizeof(resourceMessage)){
        ...
        if (myBatteryLevel.available == 0){
          led.show_in_bar(Bar_Device, 0, 0);
        }
        else {        
          led.show_in_bar(Bar_Device, 0, map(myBatteryLevel.available, 0, myBatteryLevel.total, 1, 10));
        }
      }
      break;
    case SF_MESSAGE:  ////Vessel total Solid fuel
      if (msgSize == sizeof(resourceMessage)){
        ...
        if (mySFuel.available == 0){
          ...
          led.show_in_bar(Bar_Device, 3, map(mySFuel.available, 0, mySFuel.total, 1, 10));
        }
      }
      break;  
    case LF_MESSAGE:  ////Vessel total Liquid fuel
      ...
      if (myLFuel.available == 0)
      {
        ...
        led.show_in_bar(Bar_Device, 1, map(myLFuel.available, 0, myLFuel.total, 1, 10));
      }
    }
      break;
    case ACTIONSTATUS_MESSAGE:  ////Vessel total Solid fuel
      if (msg[0] & SAS_ACTION)
      {
        led.setLed(1, 7, 1, true);      
      }
      else
      {
        led.setLed(1, 7,1,false);
      } 
    break;
  }
