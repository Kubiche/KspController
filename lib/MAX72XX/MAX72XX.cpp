...
{ 
  if ((value >= 0) && (value <= 10))
  {
    ...
    unsigned int barlevel = fullbar[(bar - 1)] << shifted;  // shift the bits left to show the desired level
    ...
    digit[device][(bar-1)] &= barMask[(bar-1)]; // Apply LSBits of actual mask to the top byte of the bar 
    digit[device][(bar)] |= barlevel;     // Apply the LSBits of the value to the top byte of bar
    setLedReg(0, (bar), digit[device][bar]); // Send the new value of the top byte of the bar to the proper digit
    digit[device][(bar - 1)] &= barMask[bar] >> 8;      // Apply MSBits of mask to the bottom byte of the bar
